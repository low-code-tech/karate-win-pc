...
- ①vwOpenPlayerのタイトル点数を 赤対青 に反転（v1.2のカード修正と同様、m.scoreは青対赤形式のため）
- ②ハイライトシークの基準を変更: 試合タイマー経過秒(sec) → 録画開始からの実時間(vsec)
  - buildMatchList: 最初の'Start'イベントのat(壁時計ISO)を cur.recStartIso として捕捉（録画はdS内のcamStartRecで開始されるため一致）
  - 各ハイライトに vsec = (Date.parse(e.at) - Date.parse(recStartIso))/1000 を付与
  - カード行▶ / パネル▶ / アクティブ行判定は vsec優先(h.vsec ?? h.sec)。フォルダ取込等でStartログがない場合はsecにフォールバック
  - 背景: タイマー停止中も録画は継続し、±1秒補正は試合時計だけを動かすため、secでは動画位置とずれる（実例: 39秒の動画に60秒後のハイライト）
- ③vwSeekToに動画長クランプ追加（duration-1超は末尾-3秒へ+トースト通知）。vwOnCanPlayも同経路に統一
- 検証: dS→3秒待ち→得点 / -1秒×10→2秒→得点 / 停止3秒→再開→得点 で sec=3,15,16 に対し vsec=3,5,9 と正しく算出、999秒シークは68秒(末尾)へクランプ
- 現行ファイル: karate-all-in-one_v1.4.html ／ マニュアル Ver.1.4


### v1.5（判定を得点に反映しない仕様へ変更）— 2026-08-02
- 課題: 判定の「決定」(commitHantei)が SC を旗数で書き換え、タイマー終了時の点数と食い違う
- 改修: commitHantei から SC書き換え(updSc含む)を削除。alog('Hantei', 旗数+主審+集計)とトースト案内のみ
- 判定の勝敗確定は下段「判定勝ち」ボタン（selectWin('decision') が closeHantei も実施、従来どおり）
- 結果表・Excel・閲覧カードはScoreログ累積のため元々旗数を含まず、表示(SC)側の書き換え廃止で全経路が一致
- 検証: 1対1同点→旗(赤2+主審赤,青1)→決定→得点1対1のまま→判定勝ち→結果行 score'1 対 1'/理由'判定勝ち'/Hanteiログ記録を確認
- 現行ファイル: karate-all-in-one_v1.5.html ／ マニュアル Ver.1.5


### v1.6（閲覧カードのハイライトボタン削除）— 2026-08-02
- vwRenderCards から「ハイライトシーン」トグルボタン・0件プレースホルダ・展開リスト(hl-list-${idx}、フッター合計含む)・vwToggleHL を削除
- ハイライト件数は「▶ 試合動画」ボタンのサブラベルに統合（ローカルファイル／ハイライトN件）
- 注意: 旧カードリストにはYouTubeのシーン別ジャンプリンク(hUrl)があった。ローカル動画がなくYouTubeのみの試合は、現状シーン別ジャンプ手段がない（試合全体リンクのみ）。要望が出たらYouTubeのみの試合に限りリスト復活を検討
- 現行ファイル: karate-all-in-one_v1.6.html ／ マニュアル Ver.1.6
